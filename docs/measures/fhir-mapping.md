...
| Visit (V File encounter) | `Encounter` — `class` from v3-ActCode (`AMB` ambulatory incl. day surgery/observation, `IMP` hospitalization, `VR` telemedicine — the User Population service categories), `type.coding` system `https://terminology.lakeraven.com/CodeSystem/ihs-clinic-code` (RPMS clinic code), `period.start` |
